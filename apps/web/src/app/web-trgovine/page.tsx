...
const stores = [
  { name: 'TechStore.hr', desc: 'Vodeća online trgovina za elektroniku i gadgete u Hrvatskoj.' },
  { name: 'ModaShop', desc: 'Modna web trgovina s ponudom domaćih i svjetskih brendova.' },
  { name: 'ZeleniVrt', desc: 'Sve za vrt i dom — sjemenke, alati i dekoracije.' },
  { name: 'PetLjubimac', desc: 'Online pet shop s hranom i opremom za kućne ljubimce.' },
  { name: 'SportMax', desc: 'Sportska oprema i odjeća za sve vrste sportova.' },
  { name: 'BioOaza', desc: 'Organska hrana i eko proizvodi za zdraviji život.' },
];
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-primary to-[#152a4a] py-20 text-white">
        <div className="mx-auto max-w-6xl px-6">
          <h1 className="font-heading text-4xl font-bold md:text-5xl">Web trgovine</h1>
          <p className="mt-3 text-lg text-white/70">Članovi udruge - web trgovci</p>
        </div>
      </section>
      ...
      <section className="border-b border-[#E2E8F0] bg-white">
        <div className="mx-auto max-w-6xl px-6 py-5">
          <div className="relative sm:max-w-md">
            ...
            <input
              type="text"
              placeholder="Pretraži web trgovine..."
              value={search}
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((store) => (
              ...
              >
                {/* Banner */}
                <div className="flex h-32 items-center justify-center bg-bg-section text-text-secondary">
                ...
                <div className="relative px-6 pb-6 pt-10">
                  ...
                  <h3 className="text-lg font-semibold text-text-heading">{store.name}</h3>
                  <p className="mt-1 text-sm text-text-body">{store.desc}</p>
                  <Link
                    href="#"
          ...
          {filtered.length === 0 && (
            <p className="py-12 text-center text-text-secondary">Nema pronađenih web trgovina.</p>
          )}
        </div>
      </section>
    </>
