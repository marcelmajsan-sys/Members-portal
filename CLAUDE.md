...
- **User↔Member je 1:N (srpanj 2026.)**: `Member.userId` NIJE unique — isti email smije imati više članstava (više webshopova), i za ISTU tvrtku. Webshop članstva je **`Member.website`** (prikaz/analiza: `member.website ?? company.website`). Admin "Dodaj člana": dropdown postojećih tvrtki (`GET /api/os/companies`) ili nova tvrtka (reuse po OIB-u ako postoji), polje Webshop se sprema na Member.website (za novu tvrtku i na Company.website); duplikat se blokira samo za puni trio email+tvrtka+webshop. `deleteMember` briše User tek s posljednjim članstvom. **Više webshopova = JEDAN profil (kolovoz 2026.)**: `Member.extraWebshops String[]` — dodatni webshopovi člana žive na ISTOM Member zapisu (novi profili/članstva za webshopove se više NE kreiraju; postojeća stara ostaju i prikazuju se kao legacy chipovi). Admin "+ Webshop" (kartica "Podaci o firmi" i modal "Uredi podatke") → `PATCH /api/os/members/:id/webshops {extraWebshops: string[]}` (OWNER; zamjenjuje listu, normalizira/deduplicira, glavni se izbacuje); uklanjanje ✕ na chipu. Modal "Uredi podatke" ima JEDNO polje "Webshop (glavni)" (uređuje member.website ako postoji, inače company.website; `PATCH .../profile` i dalje prima eksplicitne `memberWebsite`/`companyWebsite`) + read-only listu ostalih webshopova. Duplicirani profili se spajaju skriptom `packages/db/src/merge-duplicate-memberships.ts <email>` (zadržava najstariji, prepisuje sve povezane zapise, seli webshopove u extraWebshops; izvršeno za Mariju Mušicki 14.08.2026.).
