...
- **User↔Member je 1:N (srpanj 2026.)**: `Member.userId` NIJE unique — isti email smije imati više članstava (više webshopova/tvrtki). Portal/auth lookupi po userId koriste `findFirst` + `orderBy createdAt asc` (portal prikazuje PRVO članstvo). Admin "Dodaj člana": dropdown postojećih tvrtki (`GET /api/os/companies`) ili nova tvrtka (reuse po OIB-u ako postoji), polje Webshop (dopunjava prazan `Company.website`, ne pregazuje); duplikat se blokira samo za kombinaciju email+tvrtka. `deleteMember` briše User tek s posljednjim članstvom.
